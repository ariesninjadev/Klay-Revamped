...
function eraseCookie(name) { //WITHHELD
...
socket.on('checkExistsC', (resolve, amble) => {
  ...
  socket.on("userToIdFResult", function(exid) {
    if (resuser.data.pending.some(e => e.user == exid)) {
      alertify.error("You have already sent a request to this user or they have already sent one to you!")
    return false
    }
    ...
    if (exid == resuser.account.id) {
          ...
          if (!exid) {alertify.error("User doesn't exist!")} else {
