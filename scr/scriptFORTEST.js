...
var d
socket.emit("verifyId", "6951841742");
...
socket.on('verifyIdResult', (resuser) => {
  if (!resuser) {
    ...
    user = resuser
    socket.emit('homelogin', user); // include isp framing -- buffer tick space
...
socket.on('userdata', (resuse) => {
  ...
  if (resuser.account.status == 0) {
    if (resuser.account.metadata.bandata.bantext != "") {
      document.getElementById("overlayn").innerHTML = '<br><br><br><h1 style="text-align:center;font-size:2em;">Klay</h1><br><p style="text-align:center;">' + resuser.account.metadata.bandata.bantext + '</p><br><p style="text-align:center;font-size:0.7em;color:#999;">YOU MAY NOT CREATE A NEW ACCOUNT.<br><br>IF CAUGHT ATTEMPTING TO MAKE A NEW ACCOUNT,<br> ANY TEMPORARY BAN WILL BE MADE PERMANENT.</p>'
    } else {
      document.getElementById("overlayn").innerHTML = '<br><br><br><h1 style="text-align:center;font-size:2em;">Klay</h1><br><p style="text-align:center;">You no longer has access to Klay. No reason was provided.</p><br><p style="text-align:center;font-size:0.7em;color:#999;">YOU MAY NOT CREATE A NEW ACCOUNT.<br><br>IF CAUGHT ATTEMPTING TO MAKE A NEW ACCOUNT,<br> ANY TEMPORARY BAN WILL BE MADE PERMANENT.</p>'
    }
  } else {

    socket.on('getCMLName', (name, xc) => {
      if (name == false) {
        document.getElementById("chatsnav").innerHTML += `<a href="/chat/${xc.id}" class="panel-block" style="border-radius: 0px 0px 0px 0px !important;">Deleted User</a>`
      } else {
        document.getElementById("chatsnav").innerHTML += `<a href="/chat/${xc.id}" class="panel-block" style="border-radius: 0px 0px 0px 0px !important;">${name}</a>`
      }
      mcl -= 1;
      if (mcl == 0) {
        off()
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function scrollDownChat() {
    ...
    function scrollAnimation() {
      ...
      container.scrollTop = currentScrollTop + easing * (targetScrollTop - currentScrollTop);
      ...
      [
        ...
        h(
          ...
          [
            ...
            r ?
            h('div', {
              'class': 'chat-message-content iconuni',
  ...
  form.addEventListener('submit', function(event) {
    ...
    if (document.getElementById("chat-input").value == "/clearallchats" && resuser.account.metadata.admin) {
...
*/
  ...
  socket.on('chatData', function(datamm, myusername) {
    if (datamm != null) {
      for (const element of datamm) {
        messages.appendChild(
          createChatBubble(element.content, (element.name != user), element.time, element.r, element.rc, element.im, element.imc)
        );
      }
    }
...
console.log("Welcome to the testing page.\n\nValid Color Options:\nbg - Page Background\nfg - Panel Backgrounds/Foreground\nprimary - Main Text Color\nsent - Sent Message Box Background\nreceived - Received Message Box Background\naccent - Accent color (+ icon, send button)\ntextbar-primary - Textbar border/text color\ntextbar-secondary - Textbar border when typing\nsidebar-highlight - The color for title panels on the side bar\n\nCommand:\ncol(OPTION,COLOR)\n\nBe sure to use HEX colors!")

function col(option, color) {
  // Check if the provided option is a valid CSS property
  const validOptions = ['bg', 'fg', 'primary', 'sent', 'received', 'accent', 'textbar-primary', 'textbar-secondary', 'sidebar-highlight'];
  ...
  // Build the CSS variable name by adding '--' prefix
  const cssVariable = `--${option}`;

  // Update the CSS variable with the provided color
